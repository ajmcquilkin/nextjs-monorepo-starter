 ...
 */
const processResource = (originalResource) => new Promise((resolve, reject) => {
...
/**
  ...
  */
const migrateResources = () => new Promise((resolve, reject) => {
  let modifiedCount = 0;
  Resources.find({}).exec().then((resources) => {
    Promise.all(
      resources.map((resource) => new Promise((resolve, reject) => {
        Resources.findById(resource._id).then((toUpdateResource) => {
          processResource(toUpdateResource).then((updatedResource) => {
            modifiedCount += 1;
            updatedResource.save().then((modifiedResource) => resolve(modifiedResource));
          }).catch(() => resolve(null));
 ...
 */
const processUser = (originalUser) => new Promise((resolve, reject) => {
...
const migrateUsers = () => new Promise((resolve, reject) => {
  let modifiedCount = 0;
      ...
      users.map((user) => new Promise((resolve, reject) => {
        Users.findById(user._id).then((toUpdateUser) => {
          processUser(toUpdateUser).then((updatedUser) => {
            modifiedCount += 1;
            updatedUser.save().then((modifiedUser) => resolve(modifiedUser));
          }).catch(() => resolve(null));
        }).catch((findError) => reject(findError));
      })),
    ).then((modifiedUsers) => {
...
const migrateDB = () => new Promise((resolve) => {
